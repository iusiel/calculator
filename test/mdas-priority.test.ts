...
test("test mdas priority", async () => {
    ...
    await fireEvent.click(within(buttonsContainer).getByText("="));
    expect(
        document.body.querySelector(".calculator__display-result").innerHTML
    ).toEqual(`<span>=</span><span>${12 + 13 * 10}</span>`);
    ...
    expect(
        document.body.querySelector(".calculator__display-result").innerHTML
    ).toEqual(`<span>=</span><span>${40 / 5 - 10 + 2}</span>`);
    ...
    expect(
        document.body.querySelector(".calculator__display-result").innerHTML
    ).toEqual(`<span>=</span><span>${20 + 36 * 5 + 10 / 5}</span>`);
    ...
    expect(
        document.body.querySelector(".calculator__display-result").innerHTML
    ).toEqual(`<span>=</span><span>20.6</span>`);
});
