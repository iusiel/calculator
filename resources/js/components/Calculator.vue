    ...
    methods: {
        numberPressAllowed(number) {
            ...
            const predictedNumber =
                lastElement[0] !== undefined
                    ? `${lastElement[0]}${number}`
                    : number;

            if (
                this.currentEquation.length <= 1 &&
                ((parseFloat(predictedNumber) == 0 &&
                    predictedNumber.toString().length == 1) ||
                    isNaN(predictedNumber))
            ) {
                return false;
            }
        ...
        operatorPress(operator) {
            ...
            if (
                ...
                this.documentElements.length === 0
            ) {
                const span = document.createElement("span");
                span.classList.add("calculator__display-number");
                span.innerText = 0;
        ...
        equalsPress() {
            ...
            while (this.currentEquation.length > 2) {
                ...
                let percentIndex =
                    this.currentEquation.findIndex(percentSymbol);
                if (percentIndex !== -1) {
                ...
                let divisionIndex =
                    this.currentEquation.findIndex(divisionSymbol);
                if (divisionIndex !== -1) {
                ...
                let multiplyIndex =
                    this.currentEquation.findIndex(multiplicationSymbol);
                if (multiplyIndex !== -1) {
                ...
                let additionIndex =
                    this.currentEquation.findIndex(additionSymbol);

                const subtractionSymbol = (element) => element == "-";
                let subtractionIndex =
                    this.currentEquation.findIndex(subtractionSymbol);
                ...
                if (additionIndex !== -1 || subtractionIndex !== -1) {
                    if (
                        ...
                        subtractionIndex < additionIndex &&
                        subtractionIndex !== -1
                    ) {
        ...
        dotPress() {
            ...
            const lastElement = this.currentEquation.pop();
            const newElement =
                lastElement !== undefined ? `${parseFloat(lastElement)}.` : ".";
        ...
        clearPress() {
            const lastDocumentElement = this.documentElements.slice(-1);
            if (
                lastDocumentElement[0] !== undefined &&
                lastDocumentElement[0].nodeName !== "DIV"
            ) {
...
<template>
    <div class="calculator-cointainer">
        ...
        <div class="buttons-container">
            <div class="column">
                <calculator-button
                    v-on:button-pressed="clearPress"
                    press="Escape"
                    label="C"
                ></calculator-button>
            </div>
            <div class="column">
                <calculator-button
                    v-on:button-pressed="deletePress"
                    press="Backspace"
                    label="Delete"
                ></calculator-button>
            </div>
            <div class="column">
                <calculator-button
                    v-on:button-pressed="percentPress"
                    press="%"
                ></calculator-button>
            </div>
            <div class="column">
                <calculator-button
                    v-on:button-pressed="operatorPress"
                    press="/"
                    label="÷"
                ></calculator-button>
            </div>
            <div class="column">
                <calculator-button
                    v-on:button-pressed="numberPress"
                    press="7"
                ></calculator-button>
            </div>
            <div class="column">
            ...
            <div class="column">
                <calculator-button
                    v-on:button-pressed="dotPress"
                    press="."
                ></calculator-button>
            </div>
            <div class="column column-equals">
                <calculator-button
                    v-on:button-pressed="equalsPress"
                    press="="
                ></calculator-button>
            </div>
        </div>
